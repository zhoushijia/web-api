window.addEventListener('load', function () {
  ...
  for (var i = 0; i < cut.length; i++) {
    ...
    cut[i].onmouseenter = function () {
      ...
      img1.src = '../images/s' + this.index + '.jpg'
      dtp.src = '../images/e-b' + this.index + '.png'
  ...
  pol.onclick = function () {
    ...
    dtp.src = '../images/e-b2.png'
  }
  por.onclick = function () {
    this.classList.add('currentColor')
    pol.classList.remove('currentColor')
    img1.src = '../images/s1.jpg'
    dtp.src = '../images/e-b1.png'
  }

  // 头部购物车商品数量
  // $('.shop span').text(localStorage.getItem('amount'))
  // 商品信息存入本地存储
  //   获得本地存储中的comments数据
  function getProduct() {
